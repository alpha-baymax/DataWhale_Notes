...
def train(hyp, tb_writer, opt, device):
    ...
    scaler = None
    use_amp = True if opt.pyamp else False
    if use_amp:
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        if ckpt.get('amp') is not None and scaler is not None:
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size // WORLD_SIZE * 2, gs, opt, hyp=hyp, augment=False,
    ...
    t0 = time.time()
    # nni:4
    nw = max(hyp['warmup'] * nb, 1e3)  # number of warmup iterations, max(3 epochs, 1k iterations)
    # nni:4
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                # Plot 
                # Original code, you can uncomment it if need to plot.
                # if ni < 3:
            ...
            if opt.iters != -1 and i >= opt.iters -1 :
                break
            metric_collector.place()
            # end batch ------------------------------------------------------------------------------------------------
        # Scheduler
        ...
        if rank in [-1, 0]:
            ...
            if (not opt.notest or final_epoch) and not os.getenv("BENCHMARK_LOG"):  # Calculate mAP
                results, maps, times = test.test(opt.data,
                                                 batch_size=total_batch_size // WORLD_SIZE * 2,
                                                 imgsz=imgsz_test,
                ...
                if tb_writer:
                    ...
                    for x, tag in zip(list(mloss[:-1]) + list(results), tags):
                        # nni: 2
                        if tag == 'metrics/mAP_0.5:0.95':
                            nni.report_final_result(score_dict['map_0.5:0.95'])
                        # nni: 2
            ...
            # Save model
            save = ((not opt.nosave) or (final_epoch and not opt.evolve)) and not os.getenv("BENCHMARK_LOG")
            if save:
                ckpt = {'epoch': epoch,
                        'best_fitness': best_fitness,
                        'model': ema.ema.module if hasattr(ema, 'module') else ema.ema,
                        'optimizer': None if final_epoch else optimizer.state_dict(),
                        'amp': None if scaler is None else scaler.state_dict()}

                # Save last, best and delete
                os.makedirs(f'./weights/{device.type}', exist_ok=True)
                torch.save(ckpt, f'./weights/{device.type}/last.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pyamp', dest='pyamp', action='store_true', help='use torch amp to training')
    ...
    if opt.device == 'mlu':
        assert torch.is_mlu_available(), f'MLU unavailable, please check!'  # check availability
    ...
    if device.type == 'cpu':
        ...
        # DDP mode
        if device.type == "mlu":
            ct.set_device(opt.local_rank)
            device = torch.device('mlu', opt.local_rank)
            dist.init_process_group(backend='cncl', init_method='env://')  # distributed backend
        else:
    ...
    hyp['lr0'] = tuner_params["lr0"]
    hyp['optimizer'] = tuner_params['optimizer']
    hyp['momentum'] = tuner_params['momentum']
    hyp['weight_decay'] = tuner_params['weight_decay']
    hyp['iou_t'] = tuner_params["iou_t"]
    hyp['hsv_s'] = tuner_params["hsv_s"]
    hyp['hsv_v'] = tuner_params["hsv_v"]
    hyp['translate'] = tuner_params["translate"]
    hyp['scale'] = tuner_params["scale"]
    hyp['fl_gamma'] = tuner_params["fl_gamma"]
    hyp['warmup'] = tuner_params["warmup"]
    # nni: 3

    # Train
    if not opt.evolve:
        print("opt.local_rank", opt.local_rank
        if dist.get_local_rank() in [-1, 0]:
